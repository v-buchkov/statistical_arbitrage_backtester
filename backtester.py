...
from src.technical.search import binary_search_time_series
from src.file_functions.preprocessing import get_asset_returns, get_asset_prices
...
class Backtester:
    ...
    @staticmethod
    def _get_backtesting_dataset(prices: List[Tuple[dt.datetime, float]], date_start: dt.datetime,
                                 date_end: dt.datetime) -> List[Tuple[dt.datetime, float]]:
        """
        Returns (timestamp, price) data for the specified time interval.
        ...
        """
        return binary_search_time_series(time_series=prices, date_start=date_start, date_end=date_end)
    ...
    def _get_realized_vol(self, source: str, date_start: dt.datetime, date_end: dt.datetime) -> Union[float, None]:
        """
        ...
        returns_for_vol = binary_search_time_series(time_series=list(asset_returns.items()),
                                                    date_start=date_start, date_end=date_end)

        if returns_for_vol is None:
            return None
        else:
            return calculate_vol_realized([point[1] for point in returns_for_vol], delta_seconds=self.delta_seconds)
