...
class BacktesterOffshoreLocalRealized:
    ...
    def __init__(self, delta_hedge_significance: float, vol_diff_significance: float, asset: str,
                 datetime_start: dt.datetime, datetime_end: dt.datetime, rf_base_ccy: float, rf_second_ccy: float,
                 onshore_spread: float, offshore_spread: float):
        """
        ...
        None
        """
        self.delta_significance = delta_hedge_significance
        self.vol_difference = vol_diff_significance
        ...
        for point in backtest_data[:-1]:
            ...
            if (not np.isnan(vol_realized)) and (vol_realized != 0):
                ...
                if not np.isnan(delta - delta_old):
                    ...
                    # Enter the trade to hedge delta, only if delta difference is large enough (HYPERPARAMETER)
                    if abs(delta_hedge_cost * (1 + self.delta_significance)) >= spread:
    ...
    def backtest(self, days_strategy: int) -> List[float]:
        ...
        for t in range(1, days_backtesting_period - days_strategy):
            ...
            if (date_start.date() in onshore_available_days) and (date_start.date() in offshore_available_days):
                ...
                if vol_offshore - vol_onshore > self.vol_difference:
                ...
                elif vol_offshore - vol_onshore < -self.vol_difference:
